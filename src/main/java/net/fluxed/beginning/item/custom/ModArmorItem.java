...
public class ModArmorItem extends Item {
    private static final Map<ArmorMaterial, List<MobEffectInstance>> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, List<MobEffectInstance>>())
                    .put(ModArmorMaterials.MODULARIUM_ARMOR_MATERIAL,
    ...
    private void evaluateArmorEffects(Player player) {
        for(Map.Entry<ArmorMaterial, List<MobEffectInstance>> entry : MATERIAL_TO_EFFECT_MAP.entrySet()) {
            ...
            if(hasPlayerCorrectArmorOn(mapArmorMaterial, player)) {
                if(mapArmorMaterial==ModArmorMaterials.MODULARIUM_ARMOR_MATERIAL) {
                    ItemStack stack = player.getItemBySlot(EquipmentSlot.MAINHAND);
                        if (stack.is(ModItems.MODULAR_SWORD.toStack().getItem())) {
                            addEffectToPlayer(player, List.of(new MobEffectInstance(MobEffects.RESISTANCE, 1, 1)));
                        } else if (stack.is(ModItems.MODULAR_AXE.toStack().getItem())) {
                            addEffectToPlayer(player, List.of(new MobEffectInstance(MobEffects.STRENGTH, 1, 0)));
                        } else if (stack.is(ModItems.MODULAR_SPEAR.toStack().getItem())) {
                            addEffectToPlayer(player, List.of(new MobEffectInstance(MobEffects.SPEED, 1, 1)));
                        } else if (stack.is(ModItems.MODULAR_SHOVEL.toStack().getItem())) {
                            addEffectToPlayer(player, List.of(new MobEffectInstance(MobEffects.JUMP_BOOST, 1, 2)));
                        } else if (stack.is(ModItems.MODULAR_PICKAXE.toStack().getItem())) {
                            addEffectToPlayer(player, List.of(new MobEffectInstance(MobEffects.HASTE, 1, 0)));
                        } else if (stack.is(ModItems.MODULAR_HOE.toStack().getItem())) {
                            addEffectToPlayer(player, List.of(new MobEffectInstance(MobEffects.REGENERATION, 1, 0)));
                        }
                } else {
